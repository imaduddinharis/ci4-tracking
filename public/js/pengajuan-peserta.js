...
$(document).ready(function() {
    table = $('#table-pilih-peserta').DataTable({
    ...
    table.on('select', function(e, dt, type, indexed) {
        var tblData = table.rows('.selected').data();
        if (tblData.length > 0) {
            console.log(tblData.length);
            $('#simpan-peserta').prop("disabled", false);
        } else if (tblData.length === 0) {
            $('#simpan-peserta').prop("disabled", true);
        }
    });

    table.on('deselect', function(e, dt, type, indexed) {
        var tblData = table.rows('.selected').data();
        if (tblData.length > 0) {
            $('#simpan-peserta').prop("disabled", false);
        } else if (tblData.length === 0) {
...
$('#simpan-peserta').on('click', function() {
    ...
    $.each(tblData, function(i, val) {
        ...
        kodeunik = lp.concat(dump[1]);
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "http://api-bulog.sobatteknolog.com/join_diklat",
            "method": "POST",
            "headers": {
